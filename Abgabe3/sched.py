...
def schedule():
  ...
  #hier muessen wir den kuerzesten Prozess auswählen
    #choice = runqueue[0]
    length = len(runqueue)
    shortest=[]
    shortest.append(runqueue[0])
    shortest.append(get_endtime(runqueue[0])-get_starttime(runqueue[0]))
    for i in range(1,length):
        if(get_endtime(runqueue[i])-get_starttime(runqueue[i]) < shortest[1]):
            shortest[0]=runqueue[i]
    choice=shortest[0]
  else:
